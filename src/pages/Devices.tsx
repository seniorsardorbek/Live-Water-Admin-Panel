  ...
  useEffect(() => {
    setInitialRecords(() => {
      return devices.filter((item) => {
        return (
          item.seriya.toLowerCase().includes(search.toLowerCase()) ||
            ...
            <DataTable
              ...
              columns={[
                ...
                },
                {
                  ...
                  render: ({ seriya, id }) => (
                    <span>
                      <div className="text-primary underline hover:no-underline font-semibold">{`${seriya}`}</div>
                    </span>
                  ),
                },
                {
                ...
                {
                  accessor: "Yangilangan",
                  sortable: false,
                  render: ({ date }) => <span>{getHourAndMinutesFromTimestamp(date)} {getDateFromTimestamp(date)}</span>,
                },
                {
                  accessor: "Signal",
                  sortable: false,
                  render: ({ signal }) => (
                    <div className="whitespace-nowrap flex items-center gap-2">
                      {" "}
                      {signal  ? <GreenDot /> : <RedDot />}{" "}
                      {signal ? "Yaxshi" : "Signal yo'q"}{" "}
                    </div>
                  ),
                },
